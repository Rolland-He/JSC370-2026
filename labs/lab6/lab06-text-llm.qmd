...
# Rolland (Xucheng) He 1008905909 GitHub repo (branch: lab6): https://github.com/Rolland-He/JSC370-2026/tree/lab6
...
- Surgery is the most common specialty, followed by Consult - History and Phy., Cardiovascular / Pulmonary, Orthopedic and Radiology.
...
- Top 20 most frequent words are basically common words like the, and, was, of, to etc. 
- It doesn't give us a lot of useful information about the text.
- The only useful information we get from the top 20 tokens is that the word "patient" appears a lot.
...
- After we remove stopwords, we got a lot more useful information like "procedure", "left/right", "blood", "anesthesia", "performed" etc.
- Yes, it gives a better idea.
...
# Add custom stopwords
additional_stopwords = {'right', 'left', 'without'}  # Hint: add 3-4 clinical filler words
...
- There are 806 clinical stopwords.
- I removed "right", "left", "may" as additional stopwords.
- The results differ significantly. The clinical stopwords give us a lot more useful information. There are loads of adv. like almost or my in clinical stopwords. Removing those would give us a better understanding of what the text is really about.
---
...
- Yes. Some top bigrams like "operating room" or "blood pressure" seems like a common combination in medical context.
...
- Before "blood", the most frequent words are estimated, signs, white, red etc.
- After "blood", the most frequent words are pressure and loss.
- Both of the above align well like a common combination.
...
(ggplot(specialty_sentence_stats, aes(x='reorder(medical_specialty, avg_sentence_length)', y='avg_sentence_length'))
# Hint: aggregated by specialty
...
- Yes, we can see that the sentence length is generally between 17.5 and 25 words. 
- Speech - language has the longest average sentence length.
- Hospice - Palliative Care has the shortest average sentence length.
...
- Based on the 5 topics above, they are quite interpretable but there are some overlap, and I'm not too familiar with medical terms so I'm not sure if the topics are clearly distinguished enough.
  - Topic 1: pain, history, symptoms, day, year
  - Topic 2: procedure, incision, anesthesia, skin, performed
  - Topic 3: normal, negative, blood, chest, heart
  - Topic 4: cervical, disc, fracture, bone, C5, C6, L5
  - Topic 5: artery, coronary, catheter, pulmonary, vein
- The 5 topics provide good granularity and capture different medical specialties well.
...
- 
  - Topic 2 → Surgery: 793 out of 1088 surgery cases are in Topic 2
  - Topic 5 → Cardiovascular/Pulmonary: 178 out of 371 cases are in Topic 5
  - Topic 3 → Consult - History and Phy.: 355 out of 516 cases are in Topic 3
  - Topic 2 & 4 → Orthopedic: Topic 2 (176 cases) and Topic 4 (84 cases)

  - Also notice that Radiology is more distributed across topics.

- Yes, the topics align with the medical specialties.
